...
function LoginForm() {
  ...
  const onSubmit = async (data: { email: string; password: string }) => {
    ...
    if (res?.error) { toast.error('Invalid email or password'); setLoading(false); return; }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card p-8">
            ...
            <div>
              ...
              <div className="relative">
                <input {...register('password', { required: 'Password required' })}
                  type={showPw ? 'text' : 'password'} className="input-field pr-10" placeholder="••••••••" autoComplete="current-password" />
          ...
          <button onClick={() => signIn('google', { callbackUrl })}
            className="w-full flex items-center justify-center gap-3 py-3 border border-gray-200 rounded-xl hover:bg-gray-50 transition-colors text-sm font-medium text-gray-700">
