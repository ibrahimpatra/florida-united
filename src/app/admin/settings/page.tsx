...
export default function AdminSettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (snap.exists()) setSettings({ ...DEFAULT_SETTINGS, ...snap.data() as Partial<Settings> });
      } catch { /* Firestore not yet configured — use defaults */ }
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await setDoc(doc(db, SETTINGS_DOC), { ...settings, updatedAt: new Date().toISOString() });
      toast.success('✅ Settings saved — changes are live across the site!');
    } catch {
      localStorage.setItem('storeSettings', JSON.stringify(settings));
      toast.success('Saved locally (connect Firebase to persist across devices)');
    }
    setSaving(false);
  };
