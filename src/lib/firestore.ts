...
import {
  ...
  getCountFromServer, runTransaction, QueryConstraint, or,
...
export async function getAllOrders(
  ...
  const offset = (page - 1) * pageSize;
  const snap = await getDocs(query(ordersCollection, ...constraints, limit(pageSize + offset)));
...
export async function addReview(data: Omit<Review, 'id'>): Promise<string> {
  ...
  const newReviews = [...allReviews, { ...data, id: ref.id }];
...
export async function validateCoupon(code: string, orderAmount: number): Promise<{ valid: boolean; coupon?: Coupon; discount?: number; error?: string }> {
  ...
  if (orderAmount < coupon.minOrderAmount) return { valid: false, error: `Minimum order amount is $${coupon.minOrderAmount}` };
...
export async function getDashboardStats(): Promise<DashboardStats> {
  ...
  const allOrders = ordersSnap.docs.map((d) => toData<Order>(d)!);
  ...
  const thisRevenue = thisMonth.filter(o => o.paymentStatus === 'paid').reduce((s, o) => s + o.total, 0);
  const lastRevenue = lastMonth.filter(o => o.paymentStatus === 'paid').reduce((s, o) => s + o.total, 0);
  const revenueChange = lastRevenue ? ((thisRevenue - lastRevenue) / lastRevenue) * 100 : 100;
  const ordersChange = lastMonth.length ? ((thisMonth.length - lastMonth.length) / lastMonth.length) * 100 : 100;
