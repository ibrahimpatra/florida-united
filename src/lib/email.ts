  ...
  const content = `
    ...
    <table style="margin:20px 0;">
      ...
      <tfoot>
        ...
        ${order.discount > 0 ? `<tr><td colspan="3" style="color:#16a34a">Discount ${order.couponCode ? `(${order.couponCode})` : ''}</td><td style="text-align:right;color:#16a34a">-$${order.discount.toFixed(2)}</td></tr>` : ''}
        <tr><td colspan="3">Shipping</td><td style="text-align:right">${order.shippingCost === 0 ? '<span style="color:#16a34a">FREE</span>' : `$${order.shippingCost.toFixed(2)}`}</td></tr>
        <tr><td colspan="3">Tax</td><td style="text-align:right">$${order.tax.toFixed(2)}</td></tr>
        <tr class="total-row"><td colspan="3" style="text-align:left">TOTAL</td><td style="text-align:right;color:${BRAND_COLOR};font-size:18px">$${order.total.toFixed(2)}</td></tr>
      </tfoot>
    </table>
  ...
  const content = `
    ...
    <div class="info-card">
      <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:8px;">
        ...
        <div><h4>Total</h4><p style="font-weight:800;font-size:18px;color:#16a34a">$${order.total.toFixed(2)}</p></div>
